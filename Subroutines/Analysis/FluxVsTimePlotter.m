function [h,hrhs] = ...
    FluxVsTimePlotter(JA_rec, JC_rec, JT_rec,TimeRec,AL,CL,Bt,N,Ka,nu)
...
TitStr = sprintf('A: AL = %.1e CL = %.1e Bt = %.1e KD = %.1e nu = %.1e',AL,CL,Bt,Ka,nu);
xlabel('Time'); ylabel('J_A');title(TitStr);

subplot(3,1,2)
plot(TimeRec,JcL_time,TimeRec,JcM_time,TimeRec,JcR_time);
legend('Left end', 'Middle', 'Right end')
TitStr = sprintf('C: AL = %.1e CL = %.1e Bt = %.1e KD = %.1e nu = %.1e',AL,CL,Bt,Ka,nu);
...
legend('Left end', 'Middle', 'Right end')
TitStr = sprintf('Flux Total=A+C AL = %.1e CL = %.1e Bt = %.1e KD = %.1e nu = %.1e',AL,CL,Bt,Ka,nu);
xlabel('Time'); ylabel('J_T');title(TitStr);

hrhs = figure(3);
plot(TimeRec,JaR_time,TimeRec,JcR_time,TimeRec,JtR_time)
legend('A','C','A+C','Location','best')
titlestr = sprintf('Flux at right hand boundary nu = %.2e Ka = %.2e',...
    nu,Ka);
